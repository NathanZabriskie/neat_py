...
class Genome:
    ...
    def mutate(self):
        gen_out = self.copy()
        for conn in (gen_out.connections[x] for x in gen_out.connections):
            rnd = random.random()
            if rnd < _CONN_PERTURB_CHANCE:
                conn.perturb()
            else:
                conn.reset_weight()

    def add_node(self, added_connections):
        split_conn = random.choice(self.connections)
        ...
        new_incoming = Connection(from_node=split_conn.from_node,
                                  ...
                                  ID=0) # Need to check if this connection has already been added

        new_incoming.weight = split_conn.weight
    ...
    def add_connection(self, added_connections):
        ...
        newCon.is_recurrent = self.check_recurrent()

    def check_recurrent(self):
        visited = set()
        stk = set()
        for node_id in self.nodes:
            if self.has_cycle(node_id, visited, stk):
                return True

        return False

    def has_cycle(self, node_id, visited, stk):
        if node_id not in visited:
            ...
            for inc in (self.connections[x] for x in incoming_connections):
                if inc.is_recurrent: # only want to see if added new cycle.
                    continue
                if (inc.to_node not in visited and
                        self.has_cycle(inc.to_node, visited, stk)):
                    return True
                elif inc.to_node in stk:
                    return True
